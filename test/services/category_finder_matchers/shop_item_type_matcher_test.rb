...
class ShopItemCategoryMatcherTest < ActiveSupport::TestCase
  ...
  test 'find_best_match returns nil for blank title' do
    ...
    assert_nil CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: emptyTitleShopItem,
                                                                   sim: 0.15).find_best_match
    nilTitleShopItem = ShopItem.new(title: nil, url: 'www.example.com', place: @place)
    assert_nil CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: nilTitleShopItem,
                                                                   sim: 0.15).find_best_match
    spaceTitleShopItem = ShopItem.new(title: '   ', url: 'www.example.com', place: @place)
    assert_nil CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: spaceTitleShopItem,
                                                                   sim: 0.15).find_best_match
  end
  ...
  test 'find_best_match returns exact match case insensitive title' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: lowerCaseTitleShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @wine_category, result

    upperCaseTitleShopItem = ShopItem.new(title: 'WINE', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: upperCaseTitleShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @wine_category, result

    camelCaseTitleShopItem = ShopItem.new(title: 'Wine', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: camelCaseTitleShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @wine_category, result
  end

  test 'find_best_match on realistic shop item title' do
    relasticBreadcrumbShopItem = ShopItem.new(title: 'Sungold Evaporated Milk',
                                              breadcrumb: 'Shop > Grocery > Beverages > Milks , Evaporated, Condensed , Powdered, Shelf Stable > Sungold Evaporated Milk', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: relasticBreadcrumbShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @evaporated_milk_category, result
  end

  test 'find_best_match handles plural variations' do
    pluralBreadcrumbShopItem = ShopItem.new(title: 'Sungold Evaporated Milks',
                                            breadcrumb: 'Shop > Grocery > Beverages > Milks , Evaporated, Condensed , Powdered, Shelf Stable > Sungold Evaporated Milk', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: pluralBreadcrumbShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @evaporated_milk_category, result
  end

  test 'find_best_match from breadcrump title' do
    cannedTomatoesShopItem = ShopItem.new(title: 'Hunts Tomatoes Diced 8 14.5',
                                          breadcrumb: 'Shop > Grocery > Canned Goods, Soups, & Broths > Canned Vegetables > Hunts Tomatoes Diced 8 14.5', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: cannedTomatoesShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @tomatoes_canned, result

    freshTomatoesShopItem = ShopItem.new(title: 'Tomato Large Red Per#',
                                         breadcrumb: 'Shop > Produce > Fresh Vegetables > Tomatoes > Tomato Large Red Per#', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: freshTomatoesShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @tomatoes_fresh, result
  end

  test 'find_best_match from coffee breadcrump' do
    coffeeBreadcrumbShopItem = ShopItem.new(title: "Member's Selection Freeze Dried Instant Coffee 320 g / 11.2 oz",
                                            breadcrumb: "PriceSmart > Groceries > Coffee & Tea > Member's Selection Freeze Dried Instant Coffee 320 g / 11.2 oz", url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: coffeeBreadcrumbShopItem,
                                                                 sim: 0.15).find_best_match
  ...
  test 'find_best_match ignores size and unit information from title' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: wineTitleShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @wine_category, result

    beerTitleShopItem = ShopItem.new(title: 'Beer 12 pack', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: beerTitleShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @beer_category, result

    cheeseTitleShopItem = ShopItem.new(title: 'Cheese 500g', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: cheeseTitleShopItem,
                                                                 sim: 0.15).find_best_match
  ...
  test 'find_best_match ignores brand indicators in title' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: premiumWineShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @wine_category, result

    freshBreadShopItem = ShopItem.new(title: 'Fresh Bread', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: freshBreadShopItem,
                                                                 sim: 0.15).find_best_match
    assert_equal @bread_category, result
  end

  test 'find_best_match handles complex product titles' do
    complexTitleShopItem = ShopItem.new(title: 'Premium Organic Wine 750ml Select Grade A', url: 'www.example.com',
                                        place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: complexTitleShopItem,
                                                                 sim: 0.15).find_best_match
  ...
  test 'find_best_match handles synonyms' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: eggplantShopItem, sim: 0.15).find_best_match
    assert_equal categories(:eggplant), result
    brinjalShopItem = ShopItem.new(title: 'Brinjal', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: brinjalShopItem, sim: 0.15).find_best_match
    assert_equal categories(:eggplant), result
  end
  ...
  test 'find_best_match returns nil when no match found' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: noMatchShopItem, sim: 0.15).find_best_match
    assert_nil result
  end
  ...
  test 'find_best_match returns category with similarity score' do
    ...
    result = CategoryFinderMatchers::ShopItemCategoryMatcher.new(shop_item: wineShopItem, sim: 0.15).find_best_match
