...
class ShopItemShopItemMatcherTest < ActiveSupport::TestCase
  ...
  test 'should match organic milk category' do
    ...
    newOrganicMilkShopItem = ShopItem.new(title: 'Organic Milk 1l', url: 'www.example.com',
                                          breadcrumb: 'PriceSmart > Groceries > Dairy and Eggs', place: @place)
    result = CategoryFinderMatchers::ShopItemShopItemMatcher.new(shop_item: newOrganicMilkShopItem,
  ...
  test 'should not match gouda cheese category' do
    new_feta_shop_item = ShopItem.new(title: 'Feta', url: 'www.example.com', place: @place)
    result = CategoryFinderMatchers::ShopItemShopItemMatcher.new(shop_item: new_feta_shop_item,
                                                                 sim: 0.15).find_best_match
  ...
  test 'should not match shampoo shop items at a specific threshold' do
    ...
    result = CategoryFinderMatchers::ShopItemShopItemMatcher.new(shop_item: newShampooShopItem,
                                                                 sim: 0.15).find_best_match
