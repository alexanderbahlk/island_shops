ActiveAdmin.register ShopItem do
  ...
  controller do
    ...
    def category_collection
      @category_collection ||= [[nil, 'None']] + Category.products.pluck(:path, :id).map do |path, id|
        [id, path.split('/').join(' > ')]
      end.sort_by { |_id, breadcrumb| breadcrumb }
    end
  ...
  index do
    ...
    column :title do |shop_item|
      link_to shop_item.title, shop_item.url, target: '_blank', rel: 'noopener', referrerpolicy: 'origin',
                                              data: { shop_item_id: shop_item.id, turbo: false }
    end
    column :breadcrumb do |shop_item|
      # find ">" in the breadcrumb
      breadcrumbs = if shop_item.breadcrumb.present? && shop_item.breadcrumb.include?('>')
                      shop_item.breadcrumb.split(' > ')
                    elsif shop_item.breadcrumb.present? && shop_item.breadcrumb.include?('/')
                      shop_item.breadcrumb.split('/')
                    else
                      [shop_item.breadcrumb]
                    end
    ...
    column :image_url do |shop_item|
      if shop_item.image_url.present?
        image_tag shop_item.image_url, size: '50x50', style: 'object-fit: cover;', referrerpolicy: 'no-referrer',
  ...
  form do |f|
    f.inputs 'Shop Item Details' do
      ...
      f.input :category, label: 'Product', as: :select, collection: Category.only_products, include_blank: true,
                         input_html: { id: 'category_select' }
      f.input :image_url, placeholder: 'https://example.com/image.jpg'
      f.input :size
      f.input :unit, as: :select, collection: UnitParser::VALID_UNITS.sort.map { |unit|
        [unit, unit]
      }, include_blank: false
      f.input :product_id
  ...
  show do
    attributes_table do
      ...
      row :image_url do |shop_item|
        if shop_item.image_url.present?
          div do
            image_tag shop_item.image_url, style: 'max-width: 300px; height: auto;', referrerpolicy: 'no-referrer',
                                           crossorigin: 'anonymous'
          end
        end
      end
      ...
      table_for shop_item.shop_item_updates.order(created_at: :desc) do
        ...
        column :price do |update|
          best_in_place update, :price,
                        ...
                        class: 'bip-input-unit',
                        html_attrs: { style: 'width: 100px' },
                        display_with: ->(value) { value.present? ? number_to_currency(value) : 'N/A' }
        end
        column :stock_status do |update|
          best_in_place update, :stock_status,
                        ...
                        class: 'bip-input-unit',
                        html_attrs: { style: 'width: 100px' },
                        display_with: ->(value) { value.present? ? value : 'N/A' }
        end
  ...
  batch_action :unmark_needs_review do |ids|
    ...
    redirect_to_collection_with_filters(:notice,
                                        "#{ids.count} shop items have been unmarked as needing another review.")
  end

  batch_action :set_unit, form: lambda {
    {
    ...
    begin
      ...
      redirect_to_collection_with_filters(:notice,
                                          "Successfully set unit to '#{selected_unit}' for #{updated_count} shop items.")
  ...
  batch_action :assign_category, form: lambda {
    {
    ...
    if category_search.present? && category_id.blank?
      ...
      if found_category
        ...
        redirect_to_collection_with_filters(:alert,
                                            "No category found matching '#{category_search}'. Please try a different search term.")
        return
      end
    end
  ...
  batch_action :calculate_prices_per_unified_unit do |ids|
    ...
    redirect_to_collection_with_filters(:notice,
                                        "Price calculation job started for #{ids.count} shop items. Check back in a few minutes to see results.")
  end

  batch_action :create_and_assign_category, form: lambda {
    {
      category_title: :text,
    ...
    begin
      ...
      if parent_category && parent_category.depth >= 3
        redirect_to_collection_with_filters(:alert,
                                            "Cannot create product category under '#{parent_category.title}' - maximum hierarchy depth exceeded.")
        return
      end
      ...
      unless new_category.product?
        redirect_to_collection_with_filters(:alert,
                                            "Category '#{category_title}' is not a product category (must be at depth 3). Current depth: #{new_category.depth}")
        return
      end
      ...
      redirect_to_collection_with_filters(:notice,
                                          "#{action_message} '#{category_breadcrumb}' and assigned it to #{ids.count} shop items.")
    rescue ActiveRecord::RecordInvalid => e
      redirect_to_collection_with_filters(:alert,
                                          "Failed to create category: #{e.record.errors.full_messages.join(', ')}")
  ...
  collection_action :assign_missing_categories, method: :post do
    ...
    redirect_to_collection_with_filters(:notice,
                                        "Auto-assignment job started for #{pending_approval_count} items. Check back in a few minutes to see results.")
  end
  ...
  collection_action :assign_missing_size_unit, method: :post do
    ...
    # You can implement auto-assignment logic here or create a job
    redirect_to_collection_with_filters(:notice,
                                        "Auto-assignment job started for #{missing_count} items. Check back in a few minutes to see results.")
  end
  ...
  member_action :auto_assign_unit_size, method: :post do
    ...
    if (resource.unit.blank? || resource.unit == 'N/A') && parsed_data[:unit].present?
      resource.unit = parsed_data[:unit]
    end
    if resource.changed?
      resource.save!
