class Api::V1::UsersController < Api::V1::SecureAppController
  ...
  def login_or_create
    Rails.logger.info("Received params for login_or_create: #{params.inspect}")
    # Allow nested device_data hash
    user_params = params.require(:user).permit(:app_hash, device_data: {})
    ...
    if @current_user.nil?
      # Persist device metadata when creating a new user
      @current_user = User.new(app_hash: user_params[:app_hash], device_data: user_params[:device_data])
      ...
      # Update stored device metadata on subsequent logins if provided
      if user_params[:device_data].present?
        begin
          @current_user.update(device_data: user_params[:device_data])
        rescue StandardError => e
          Rails.logger.warn("Could not update device_data for user #{@current_user&.id}: #{e.message}")
        end
      end
  ...
  def update_tutorial_step
    if @current_user.update(tutorial_step: params[:tutorial_step])
      render json: { message: 'Tutorial step updated successfully', tutorial_step: @current_user.tutorial_step },
             status: :ok
    else
      render json: { errors: @current_user.errors.full_messages }, status: :unprocessable_content
    end
  ...
  def send_feedback
    ...
    feedback_params[:content] = feedback_params[:content].to_s.gsub(%r{<script.*?>.*?</script>}m, '')
    ...
    if feedback_params[:contact_details].present?
      feedback_params[:contact_details] = feedback_params[:contact_details].to_s.gsub(%r{<script.*?>.*?</script>}m, '')
    ...
    feedback = @current_user.feedbacks.new(content: feedback_params[:content],
                                           contact_details: feedback_params[:contact_details])
    if feedback.save
